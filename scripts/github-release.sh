...
export GITHUB_REPO=hiv-survdb-payload

info=$(github-release info --json 2>/dev/null || true)
...
Built with hivdb/hiv-survdb@${builder_local_commit}\n"
else
  ...
  if [[ "$builder_remote_commit" != "$builder_local_commit" ]]; then
    echo "Release abort: the local repository hiv-survdb seems not up-to-date.  Forgot running 'git pull --rebase' and 'git push'?" 1>&2
    exit 1
  fi
  if [[ "$remote_commit" != "$local_commit" ]]; then
    echo "Release abort: the local repository hiv-survdb-payload seems not up-to-date. Forgot running 'git pull --rebase' and 'git push'?" 1>&2
    exit 1
  fi
...
if [ ! -f "build/hiv-survdb-$VERSION.db" ]; then
  echo "Release abort: file 'build/hiv-survdb-$VERSION.db' is not found. Something wrong, please contact Philip." 1>&2
  exit 2
fi

if [ ! -f "build/hiv-survdb-$VERSION-slim.db" ]; then
  echo "Release abort: file 'build/hiv-survdb-$VERSION-slim.db' is not found. Something wrong, please contact Philip." 1>&2
  exit 2
fi

echo -e $description | github-release release --tag $VERSION --name "$title" $PRE_RELEASE --description -
sleep 10
github-release upload --tag $VERSION --name "hiv-survdb-$VERSION.db" --file "build/hiv-survdb-$VERSION.db"
github-release upload --tag $VERSION --name "hiv-survdb-$VERSION-slim.db" --file "build/hiv-survdb-$VERSION-slim.db"

if [[ "PRE_RELEASE" == "--pre-release" ]]; then
  echo "Pre-release $VERSION created: https://github.com/hivdb/hiv-survdb-payload/releases/tag/$VERSION"
else
  echo "Release $VERSION created: https://github.com/hivdb/hiv-survdb-payload/releases/tag/$VERSION"
fi
